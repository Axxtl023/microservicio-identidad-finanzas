...
export class OutboxPublisherService {
  ...
  /**
   * Worker que corre cada segundo.
   * Lee los eventos del outbox que aún no han sido publicados, los publica a RabbitMQ,
   * y marca cada uno con published_at. En caso de error incrementa attempts + last_error.
   * LOS REGISTROS NUNCA SE BORRAN (política append-only para auditoría).
   */
  ...
  async publishPending(): Promise<void> {
    ...
    for (const record of pending) {
      try {
        ...
        // Si se alcanza el límite máximo de intentos, log crítico
        if (record.attempts + 1 >= this.maxAttempts) {
          this.logger.error(
            `[outbox] CRÍTICO: Evento ${record.event_id} (${record.event_type}) alcanzó ${this.maxAttempts} intentos fallidos. Revisión manual requerida.`,
          );
        }
      }
    }
